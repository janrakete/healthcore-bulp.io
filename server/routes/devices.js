...
router.post("/:table*", async function (request, response) {
   ...
   if (tablesAllowed.includes(table)) {  // check, if table name is in allowed list
      try {
         ...
         const statementBuild = await statementBuild(table, request, "INSERT");
...
router.get("/:table*", async function (request, response) {
   ...
   if (tablesAllowed.includes(table)) {  // check, if table name is in allowed list
      try {
         ...
         const conditionBuild = await conditionBuild(table, request);
...
router.patch("/:table*", async function (request, response) {
   ...
   const requestBody  = request.body; // POST values are for data
   const requestQuery = request.query; // GET values are for condition

   if (tablesAllowed.includes(table)) {  // check, if table name is in allowed list
      try {
         data.status = "ok";

         const conditionBuild = await conditionBuild(table, requestQuery);
         if (conditionBuild.status === "ok") {
            if (conditionBuild.condition.trim() !== "") {

               const statementBuild = await statementBuild(table, requestBody, "UPDATE");
               if (statementBuild.status === "ok") {
