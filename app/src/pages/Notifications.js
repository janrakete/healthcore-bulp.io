        ...
        if (!items || items.length === 0) {
          ...
          `;
            ...
            <ion-card color="primary" data-id="${item.notificationID}" class="small">
              ...
              ${item.insightID > 0 ? `<ion-button href="/care-insights/${item.insightID}"><ion-icon slot="start" name="analytics-outline" color="tertiary"></ion-icon><ion-text color="light">${window.Translation.get("CareInsightGoTo")}</ion-text></ion-button>` : ''}
              </ion-card>
            `).join("");
        }
      }
