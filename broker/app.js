...
const statementInsertValue = database.prepare(
    "INSERT INTO mqtt_devices_values (deviceID, dateTime, dateTimeAsNumeric, property, value, valueAsNumeric, weekday, weekdaySin, weekdayCos, hour, hourSin, hourCos, month) VALUES (?, datetime('now', 'localtime'), ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"
);
    ...
    aedes.authorizePublish = function (client, packet, callback) { // execute SQL statements before MQTT messages are published
        if (client) {
            ...
            try {
                ...
                if (topic === "server/devices/values/get") { // if topic is for device values, then insert values also into mqtt_devices_values to use for alerts and related analytics
