...
| 24 | **ZigBee watchdog detects stale devices** | Connect a ZigBee device, then remove its power source. After `CONF_devicesZigBeeWatchdogTimeoutSeconds` without receiving data, the watchdog (checked every `CONF_devicesZigBeeWatchdogIntervalSeconds`) should flag the device as offline. |
| 25 | **ZigBee reconnect with exponential backoff** | Disconnect a ZigBee device (e.g. unplug it) and observe the bridge logs. The bridge should retry connection with increasing delays from `CONF_devicesZigBeeReconnectBaseDelaySeconds` up to `CONF_devicesZigBeeReconnectMaxDelaySeconds`. |
