...
| 20 | **Alerts detects unusual values** | With `CONF_alertsActive` enabled, send ~200 normal heartrate values (60–80) to build a baseline. Then send an extreme value (e.g. 250). Check the server logs, the `alerts` table, or the `/alerts` API for a new `AnomalyDetection` alert with `source = 'alerts_rule'`. |
| 21 | **Alerts publishes MQTT event** | After triggering an unusual value (see above), subscribe to `server/alerts` with an MQTT client. Verify the published message contains the alertID, type and title. |
...
| 35 | **notification action creates alert without push** | Create a scenario with a `notification` action (type: `notification`, value: title, property: summary). Trigger the scenario and verify a row appears in the `alerts` table with `source = 'scenario'` and the matching `scenarioID`, but no push notification is sent to devices. |
