...
| 20 | **Care Insights detects unusual values** | With `CONF_careInsightsActive` enabled, send ~200 normal heartrate values (60–80) to build a baseline. Then send an extreme value (e.g. 250). Check the server logs, the `care_insights` table, or the `/care-insights` API for a new `unusual_numeric_pattern` insight. |
| 21 | **Care Insights publishes MQTT alert** | After triggering an unusual value (see above), subscribe to `server/care-insights` with an MQTT client. Verify the published message contains the insightID, type, severity, and title. |
...
| 48 | **Swagger UI loads** | Open `http://localhost:9998/api-docs` in a browser. The Swagger UI should render with all documented endpoints grouped by tag (Data, Devices, Scenarios). Verify you can expand each endpoint and see its parameters, request body, and response schema. |
