...
| 25 | **ZigBee adapter reconnect with exponential backoff** | Disconnect the ZigBee adapter (e.g. unplug the USB coordinator) and observe the bridge logs. The bridge should retry the adapter connection with increasing delays from `CONF_devicesZigBeeReconnectBaseDelaySeconds` up to `CONF_devicesZigBeeReconnectMaxDelaySeconds`. |
